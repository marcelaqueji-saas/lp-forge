...
import { getAllContent, getSettings, trackLPEvent, LPContent, LPSettings, getLPBySlug } from '@/lib/lpContentApi';
import { initGA4, initMetaPixel, trackPageView } from '@/lib/analytics';
import { captureUTMParams } from '@/lib/utm';
import { Skeleton } from '@/components/ui/skeleton';

// Hero secundário component (simpler hero for internal pages)
  ...
  return (
    <section className="section-padding bg-gradient-to-br from-primary/5 to-secondary/5">
      <div className="section-container text-center">
        ...
        {subtitulo && <p className="text-xl text-muted-foreground max-w-2xl mx-auto">{subtitulo}</p>}
      </div>
    </section>
  );
};
...
const SitePublic = () => {
  ...
  // Track CTA clicks
  const handleCTAClick = useCallback((section: string, ctaType: string) => {
    if (site) {
      trackLPEvent(site.id, 'cta_click', { section, cta_type: ctaType, page: pageSlug || 'home' });
    }
  }, [site, pageSlug]);
  ...
  useEffect(() => {
    ...
    const loadSite = async () => {
      try {
        ...
        // If not found by domain and we have a slug, try loading by slug
        if (!loadedSite && siteSlug) {
          // Get site by slug from landing_pages where is_site = true
        ...
        // Load page by slug (empty string for home)
        ...
        // Track view event
        const viewKey = VIEW_TRACKED_KEY + loadedSite.id + '_' + (pageSlug || 'home');
        if (!sessionStorage.getItem(viewKey)) {
          await trackLPEvent(loadedSite.id, 'view', { page: pageSlug || 'home' });
  ...
  const getVariante = (section: string): 'modelo_a' | 'modelo_b' | 'modelo_c' => {
    return (settings[`${section}_variante`] as 'modelo_a' | 'modelo_b' | 'modelo_c') || 'modelo_a';
  };
  ...
  return (
    <div className="min-h-screen bg-background site-container">
      ...
      {page.section_order.map((section) => {
        ...
        // Add variante for components that support it
        if (!['rodape', 'hero_secundario', 'form_contato', 'cta_whatsapp', 'garantia'].includes(section)) {
          props.variante = getVariante(section);
        }

        // Add lpId for form components
        ...
        if (section === 'chamada_final') {
          props.lpId = site.id;
          props.onPrimaryCTAClick = () => handleCTAClick('chamada_final', 'primary');
        }
